...
ram_read() {
	MEM_USED=`awk '/MemTotal/ {memtotal=$2}; /MemAvailable/ {memavail=$2}; END { printf("%.0f", (100- (memavail / memtotal * 100))) }' /proc/meminfo`
}

i3status | (read line && echo "$line" && read line && echo "$line" && read line && echo "$line" && update_rate && ram_read && while :
do
	read line
	update_rate
	echo ",[{\"full_text\":\"RAM: ${MEM_USED}%\"},[{\"full_text\":\"${rate}\"},${line#,\[}" || exit 1
done)
